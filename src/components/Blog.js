...
const DisplayComments = ({comment, index}) => {
    return (
        <>
            <Row className="ml-2">Comment {index+1} : {comment.body}</Row>
        </>
    )
}
...
const DisplayBlog = () => {
    ...
    return (
        <>
            ...
            <Button color="primary" onClick={handleClick}>Comments</Button>
            {comments.length ? comments.map((c,i) => <DisplayComments comment={c} index={i}/>) : ""}
